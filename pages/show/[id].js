    ...
    const index = result.videos ? .results ? .findIndex(
        ...
        return ( <
            ...
            Header / > {!session ? ( <
                    ...
                    /div> <
                    /div> <
                    p className = "text-xs md:text-sm" > { result.release_date || result.first_air_date }• { " " } { result.number_of_seasons } { " " } { result.number_of_seasons === 1 ? "Season" : "Seasons" }• { " " } { /* {result.genres.map((genre) => genre.name + ", ")} */ } <
                    /p> <
